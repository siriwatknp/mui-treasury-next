...
```jsx
import { getHeader, getContent } from "@mui-treasury/layout";
...
```jsx
import { Header, Content } from "@mui-treasury/layout";
```
...
- does not support breakpoint config anymore (It is removed because I don't see any use case that has different inset sidebar between breakpoint. By removing this, a lot of code is reduced and more maintainable).
- use `hidden` for the breakpoints that don't want to show inset sidebar.
